...
def find_r_correlation(sx, sy, b):
    r = (sx / sy) * b
    return r
...
def find_t_r(r, n):
    t_r0 = r * (math.sqrt(n - 2))
    t_r1 = math.sqrt(1 - (r ** 2))
    t_r = t_r0 / t_r1
    return t_r
...
def get_answer(samples_x, samples_y, a, b):
    ...
    if x_int_exception(x) == 0 and y_int_exception(y) != 0:
        ...
        r_correlation = find_r_correlation(sx, sy, b)
        this_x = find_x(a, b, y, c)
        print(f"c = {c}\n"
              f"r-correlation = {r_correlation}\n"
        ...
        if r_correlation >= 0:
            print(f"Since H0: r = 0 and Ha : r != 0 \n"
                  f"r-correlation = {r_correlation}\nthe null hypothesis is rejected"
                  f"  because the correlation is non-zero."
                  f" going to next step... find T-critical")
        else:
            print(f"Since H0: r = 0 and Ha : r != 0 \n"
        ...
        t_value = find_t_r(len(samples_x), r_correlation)
        if t_value >= 1.83:
            print(f"Since H0: Tr < {t_value} and Ha : Tr >= 1.83 \n"
                  f"T-critical value is {t_value} \nthe null hypothesis is rejected"
                  f" The relationship between X and Y is statistically significant.")
        else:
            print(f"Since H0: Tr < {t_value} and Ha : Tr >= 1.83 \n"
                  f"T-critical value is {t_value}\n the null hypothesis is rejected."
                  f" The relationship between X and Y is not statistically significant.")
